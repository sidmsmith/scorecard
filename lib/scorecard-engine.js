...
/**
 * Last round index (0-based) with a non-blank score per player, or -1.
 * @param {(string | number | null | undefined)[][]} rounds
 * @param {number} playerCount
 * @returns {number[]}
 */
export function lastEnteredRoundByPlayer(rounds, playerCount) {
  ...
  for (let ri = 0; ri < rounds.length; ri++) {
    ...
    for (let ci = 0; ci < playerCount; ci++) {
      if (String(row?.[ci] ?? "").trim() !== "") last[ci] = ri;
    }
  }
  return last;
}
...
/**
 * Leader indices among players who still show a running total at round ri.
 * @param {number[]} roundTotals
 * @param {number[]} lastEntered
 * @param {number} ri
 * @returns {number[]}
 */
export function visibleLeaderIndices(roundTotals, lastEntered, ri) {
  const visible = [];
  for (let ci = 0; ci < roundTotals.length; ci++) {
    if ((lastEntered[ci] ?? -1) >= ri) visible.push(ci);
  }
