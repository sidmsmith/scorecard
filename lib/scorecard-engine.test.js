...
test("lastEnteredRoundByPlayer finds last non-blank per column", () => {
  const rounds = [
    ["1", "2", ""],
    ["", "3", ""],
    ["4", "", ""],
  ...
  assert.deepEqual(lastEnteredRoundByPlayer([["", ""]], 2), [-1, -1]);
});

test("visibleLeaderIndices only considers columns showing totals at that round", () => {
  // lastEntered [2,0,5]: at ri=2 only cols 0 and 2 show; totals 10 vs 3 → [0]
  assert.deepEqual(visibleLeaderIndices([10, 12, 3], [2, 0, 5], 2), [0]);
  // at ri=0 all three show; max 12 → [1]
  assert.deepEqual(visibleLeaderIndices([10, 12, 3], [2, 0, 5], 0), [1]);
});
